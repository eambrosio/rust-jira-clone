...
impl JiraDatabase {
    ...
    pub fn create_epic(&self, epic: Epic) -> Result<u32> {
        let mut db_content = self.read_db()?;
        let new_id = db_content.last_item_id + 1;
        db_content.epics.insert(new_id, epic);
        db_content.last_item_id = new_id;

        self.database.write_db(&db_content)?;
        Ok(new_id)
    }
    ...
    pub fn create_story(&self, story: Story, epic_id: u32) -> Result<u32> {
        ...
        parsed
            .epics
            .get_mut(&epic_id)
            .ok_or_else(|| anyhow!("could not find epic in db"))?
            .stories
            .push(new_id);
    ...
    pub fn delete_epic(&self, epic_id: u32) -> Result<()> {
        ...
        for story_id in &parsed
            .epics
            .get(&epic_id)
            .ok_or_else(|| anyhow!("could not find the epic"))?
            .stories
        {
        ...
        parsed
            .epics
            .remove(&epic_id)
            .ok_or_else(|| anyhow!("could not delete the epic"))?;
    ...
    pub fn delete_story(&self, epic_id: u32, story_id: u32) -> Result<()> {
        ...
        let story_index = epic
            ...
            .ok_or_else(|| anyhow!("story not found in epic stories vector"))?;
    ...
    pub fn update_epic_status(&self, epic_id: u32, status: Status) -> Result<()> {
        ...
        parsed
            .epics
            .get_mut(&epic_id)
            .ok_or_else(|| anyhow!("could not found the epic"))?
            .status = status;
    ...
    pub fn update_story_status(&self, story_id: u32, status: Status) -> Result<()> {
        ...
        parsed
            .stories
            .get_mut(&story_id)
            .ok_or_else(|| anyhow!("could not find the story"))?
            .status = status;
...
impl Database for JSONFileDatabase {
    fn read_db(&self) -> Result<DBState> {
        let db_state_string = read_to_string(&self.file_path)?;
