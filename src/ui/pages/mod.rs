...
impl Page for HomePage {
    fn draw_page(&self) -> Result<()> {
        ...
        for id in epics.keys().sorted() {
            ...
            let name_col = get_column_string(&epic.name, 33);
...
impl Page for StoryDetail {
    fn draw_page(&self) -> Result<()> {
        ...
        let id_col = get_column_string(&self.story_id.to_string(), 4);
        let name_col = get_column_string(&story.name, 12);
        let description_col = get_column_string(&story.description, 27);
        let status_col = get_column_string(&story.status.to_string(), 12);
